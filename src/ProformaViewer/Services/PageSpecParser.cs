...
public static class PageSpecParser {
 public static IReadOnlyList<int> Parse(string value) {
  ...
  foreach(var raw in value.Split(new[]{',',';'}, StringSplitOptions.RemoveEmptyEntries|StringSplitOptions.TrimEntries)) {
   ...
   if(x>0) { var page=Positive(token[..x]); var count=Positive(token[(x+1)..]); for(int i=0;i<count;i++) pages.Add(page); }
   else pages.Add(Positive(token));
  }
  if(pages.Count==0) throw new FormatException($"Invalid page value '{value}'."); return pages;
 }
 static int Positive(string s)=>int.TryParse(s.Trim(),out var n)&&n>0?n:throw new FormatException($"Invalid page number '{s}'.");
}
